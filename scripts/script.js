      ...
      var photoQuery;
        ...
        return msg.http('http://www.loc.gov/pictures/search/?fo=json&fa=displayed:anywhere&fi=date&q=' + photoQuery).get()(function(err, res, body) {
          ...
          if (response.search.hits > 2) {
            return msg.http('http://www.loc.gov/pictures/search/?fo=json&fa=displayed:anywhere&fi=date&sp=2&q=' + photoQuery).get()(function(err, res, body) {
            var responsePage2;
            responsePage2 = JSON.parse(body);
            let page2hits = responsePage2.results.length;
          };
            ...
            if (images.length > 0) {
            return msg.send( "http:" + photourl + "\n find date: *" + photoQuery  + "* \n photo date: " + date + "   \n photo title: " + title + "\n by: " + fixName(photographer) + "\n more info at - http:" + response.results[rando].links.item + page2hits);
          }
